package 数据结构与算法.查找算法.斐波那契查找_也叫黄金分割法_数组必须是有序的;
...
public class FibonacciSearch {
  ...
  public static int fibSearch(int[] a, int key) {
    ...
    // 因为 f[k] 值 可能大于 a 的长度，因此我们需要使用Array类，构造一个新的数组，并指向temp[]
    // 不足的部分会使用0填充
    int[] temp = Arrays.copyOf(a, f[k]);
    // 实际上需求使用a数组最后的数组后的数填充 temp
    ...
    while (low <= high) { // 只要这个条件满足，就可以找
      ...
      if (key < temp[mid]) { // 我们应该继续向数组的前面查找(左边)
        ...
        // 1. 全部元素 = 前面的元素 + 后面元素
        // 2. f[k] = f[k - 1] + f[k - 2]
        // 因为 前面有 f[k - 1]个元素，所有可以继续拆分 f[k - 1] = f[k - 2] + f[k - 3]
        // 即 在 f[k - 1] 的前面继续查找 k--
        // 即下次循环 mid = f[k - 1 - 1] - 1
        k--;
